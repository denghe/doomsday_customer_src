...
namespace Game {
	...
	inline void Stage4::Init() {
		...
		//sc->shootSpeed = 1000 / Cfg::fps;
		//sc->life = 3 * (int32_t)Cfg::fps;
		//sc->pierceCount = 0;
		//sc->pierceDelay = 0;

		{
			auto& sp = monsterGenerators.Emplace().Emplace<MonsterGen_Generic<Monster_Instantnoodles>>()
				->Init(this, 0, int32_t(Cfg::fps) * 10, 250);
			sp.healthPoint = 10;
	...
	inline void Stage4::OnRoundBegin() {}

	inline void Stage4::OnRoundEnd() {
