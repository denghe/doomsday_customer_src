...
namespace Game {
	...
	int32_t Monster_Instantnoodles::Hurt(float dmg, XY const& d) {
		dmg = std::ceilf(dmg);
		if (hp <= dmg) {
			// dead
	...
	void Monster_Instantnoodles::Draw() {
		auto q = gLooper.ShaderBegin(gLooper.shaderQuadInstance)
			.Draw(gLooper.res.monster_instantnoodles->tex->GetValue(), 1);
		q->pos = stage->camera.ToGLPos(pos);
		q->anchor = ResTpFrames::_anchor_monster_instantnoodles_;
		q->scale = scale * stage->camera.scale;
		q->radians = 0;
		q->colorplus = whiteColorEndTime >= stage->time ? 10000.f : 1.f;
		q->color = xx::RGBA8_White;
		q->texRect.data = gLooper.res.monster_instantnoodles->textureRect.data;
	}
