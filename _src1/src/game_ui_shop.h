...
namespace Game {
	...
	struct UI_BuffBag : xx::ScrollView {
		...
		void Init(Creature* creature_, int z_, XY const& position_, XY const& scale_, XY const& anchor_, XY const& size_);
	};
	...
	struct UI_BuffsContent : xx::Node {
		...
		Creature* creature{};
		UI_BuffBag* buffBag{};	// shortcut for easy access. same as parent.parent
		float itemMarginX{ cItemMargin.x };
		int32_t numCols{}, numRows{};

		UI_BuffsContent* Init(Creature* creature_, UI_BuffBag* buffBag_);
		void Refresh();
	...
	struct UI_BuffShopGoodsItem : xx::EmptyButton {
		Creature* creature{};
		void Init(Creature* creature_, BuffTypes bt, int z_, XY const& pos_, XY const& anchor_, XY const& size_);
	};

	// UI_BuffShop's children
	struct UI_BuffShopGoodsList : xx::Node {
		Creature* creature{};
		void Init(Creature* creature_, int32_t z_, XY pos_, XY size_);
		void Refresh();
	};

	struct UI_PlayerStat : xx::Node {
		Creature* creature{};
		void Init(Creature* creature_, int32_t z_, XY pos_);
		void Refresh();
	};
	...
	struct UI_BuffShop : xx::Node {
		...
		std::function<void()> OnNextBtnPressed;	// need init

		UI_BuffShop& Init(Creature* creature_);
	};
