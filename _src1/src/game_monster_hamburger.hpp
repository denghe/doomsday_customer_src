...
namespace Game {
	...
	int32_t Monster_Hamburger::Hurt(float dmg, XY const& d) {
		dmg = std::ceilf(dmg);
		if (hp <= dmg) {
			// dead
	...
	int32_t Monster_Hamburger::Update() {
	...
	void Monster_Hamburger::Draw() {
		auto q = gLooper.ShaderBegin(gLooper.shaderQuadInstance)
			.Draw(gLooper.res.monster_hamburger->tex->GetValue(), 1);
		...
		q->texRect.data = gLooper.res.monster_hamburger->textureRect.data;
	}
