...
namespace Game {
	...
	inline void MainMenu::MakeUI() {
		...
		// title
		ui->MakeChildren<xx::Label>()->Init(1, pos5 + XY{ 0, 200 }, 5, 0.5f, xx::RGBA8_White, UI::TextOf_title());
		...
		// right bottom icons ( language, qq? discord? )
		{
			...
			auto& b = ui->MakeChildren<xx::FocusImageButton>()->Init(2, pos3 + XY{ -30, 30 }, anchor3
				, 64, 5, gLooper.cfg_imgBtnNormal, gLooper.cfg_imgBtnHighlight, std::move(flag));
			b.onFocus = [] { gLooper.sound.Play(gLooper.res_sound_button_1); };
			b.onClicked = [this] {
				UI::SetNextLanguage();
				MakeUI();
